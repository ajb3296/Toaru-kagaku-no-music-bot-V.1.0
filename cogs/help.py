    ...
    async def help (self, ctx, *, arg : str  = None) :
        ...
        if arg == "GENERAL" or arg == "일반":
            ...
            if "cogs.music" in EXTENSIONS:
                ...
                embed.add_field(name=f":white_check_mark: | `{commandInt}connect`", value="> 음성 채널에 접속해요!\n> `%sjoin`, `%sc`, `%s들어와`, `%sㅊ`  을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":arrow_forward: | `{commandInt}play` [*음악 이름 혹은 Youtube 링크*]", value="> 음악을 재생해요!\n> `%sp`, `%s재생`, `%sㅔ`  을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":x: | `{commandInt}disconnect`", value="> 음성 채널을 나갑니다.\n> `%sdc`, `%s연결해제`, `%s나가` 을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":track_next: | `{commandInt}skip`", value="> 다음 곡으로 넘어갑니다!\n> `%s스킵`, `%ss`, `%sㄴ` 을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":loud_sound: | `{commandInt}vol` *(1~1000)*", value="> 음량을 조절합니다!\n> `%sv`, `%sㅍ`, `%s음량`, `%s볼륨`  을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":arrow_down_small: | `{commandInt}now`", value="> 현재 재생중인 곡을 알려드려요!\n> `%sn`, `%snp`, `%s현재재생중`, `%sㅜ` 을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":regional_indicator_q: | `{commandInt}q` *(페이지 수)*", value="> 재생목록 리스트의 *(페이지 수)*의 페이지에 있는 재생목록을 알려드려요!\n> `%s큐`, `%sㅂ` 을(를) 사용할 수도 있어요." %(commandInt, commandInt), inline=False)
                embed.add_field(name=f":play_pause: | `{commandInt}pause`", value="> 음악을 일시정지해요!\n> `%s일시정지` 을(를) 사용할 수도 있어요." %commandInt, inline=False)
                embed.add_field(name=f":twisted_rightwards_arrows: | `{commandInt}shuffle`", value="> 다음 곡부터 재생목록의 음악들이 랜덤으로 나와요\n> `%s셔플` 을(를) 사용할 수도 있어요." %commandInt, inline=False)
                embed.add_field(name=f":repeat: | `{commandInt}repeat`", value="> 현재 듣고 계시는 노래를 포함한 재생목록의 모든 노래를 반복해서 들려드려요!\n> `%sloop`,`%sl`, `%s반복`, `%sㅣ` 을(를) 사용할 수도 있어요." %(commandInt, commandInt, commandInt, commandInt), inline=False)
                embed.add_field(name=f":stop_button: | `{commandInt}stop` *(분)*", value="> 듣고 계시는 음악을 끄고 재생목록을 제거해요!\n> 분 에 분단위 시간을 넣으시면 해당 분 후 음악이 자동으로 멈춥니다!\n> `%s중지`, `%s정지` 을(를) 사용할 수도 있어요." %(commandInt, commandInt), inline=False)
                embed.add_field(name=f":clock: | `{commandInt}seek` [*+(초) 혹은 -(초)*]", value="> 음악을 명령어 뒤의 숫자만큼 초단위로 진행시간을 조절해요!\n> `%s탐색` 을(를) 사용할 수도 있어요." %commandInt, inline=False)
                ...
                embed.add_field(name=f":melon: | `{commandInt}melonplay`", value=f"> 멜론 차트 1~10위의 곡을 재생목록에 추가합니다!\n> `{commandInt}멜론재생`, `{commandInt}멜론음악`, `{commandInt}멜론차트재생` 을(를) 사용할 수도 있어요.", inline=False)
                embed.set_footer(text=BOT_NAME_TAG_VER)
                await ctx.send(embed=embed)
