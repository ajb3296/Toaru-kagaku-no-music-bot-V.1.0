    ...
    async def help (self, ctx, *, arg : str  = None) :
        ...
        if arg == "GENERAL" or arg == "일반":
            ...
            if "cogs.music" in EXTENSIONS:
                ...
                embed.add_field(name=":globe_with_meridians: | `%sfind` [*검색하실 음악명*]" %commandInt, value="> Youtube 에서 음악을 검색하고 결과를 알려드려요!\n> `%syoutube`, `%s유튜브` 을(를) 사용할 수도 있어요." %(commandInt, commandInt), inline=False)
